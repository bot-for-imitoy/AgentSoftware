...
public class TalkTo extends Tool {
    ...
    public TalkTo(Role role, RolePool pool) {
        super();
        this.role = role;
        this.pool = pool;
    }
    ...
    public String handler(Map<String, Object> args) {
        ...
        Role targetRole = pool.getRoleByName(target);
        if (targetRole == null) {
        ...
        if (targetRole.state == Types.AgentState.WAIT
            ...
            if (role != null) {
                role.journal("Replied to waiting " + targetRole.name + ": " + truncate(message, 80));
            }
        ...
        String waitingHint = "";
        if (wait && role != null) {
            waitingHint = "\n\n⚠️ " + role.name + " is waiting for your reply (wait=true). "
        ...
        Role.Task task = new Role.Task(urgency.value,
        ...
        if (wait) {
            ...
            if (targetRole.state == Types.AgentState.WAIT
                    && wouldDeadlock(pool, targetRole, role.roleId)) {
            ...
            role.journal("Sent message to " + targetRole.name + " (" + urgency.name() + ", waiting for reply): "
                    + truncate(message, 80));
            recordTalk(targetRole, message, urgency.name());
            String reply = role.talkWait(targetRole.roleId, task, null);  // wait indefinitely
        ...
        if (role != null) {
            role.journal("Sent message to " + targetRole.name + " (" + urgency.name() + "): "
                    + truncate(message, 80));
        }
        recordTalk(targetRole, message, urgency.name());
     ...
     */
    private void recordTalk(Role targetRole, String message, String urgency) {
        ChatStore store = role != null && role.system() != null
        ...
        store.record(ChatStore.KIND_TALK, group, role.roleId, role.name,
    ...
    private static boolean wouldDeadlock(RolePool pool, Role startRole, String senderId) {
